        ...
        match socket.read_frame() {
                    ...
                    || e.kind() == std::io::ErrorKind::TimedOut =>
            {
                ...
                if last_read_time.is_some() {
                    let now = Instant::now();
                    // Silently record gap start on first timeout
                    if consecutive_timeouts == 1 {
                        gap_start_time = Some(now);
                        writer_writes_at_gap_start = writer_busy.load(Ordering::Relaxed);
                    }
                    // Only log after 3+ timeouts (~30ms) to skip normal inter-frame gaps
                    if consecutive_timeouts >= 3 && consecutive_timeouts % 3 == 0 {
                        ...
                        tracing::warn!(
                            "CAN gap +{:.0}ms: timeout #{}, {} writes so far, last write {:.1}ms ago, last read id=0x{:x}",
                            gap_ms,
                            consecutive_timeouts,
                            writes_since_gap,
                            ms_since_last_write,
