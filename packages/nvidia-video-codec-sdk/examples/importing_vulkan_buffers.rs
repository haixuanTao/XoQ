...
/// `HOST_VISIBLE` and `HOST_COHERENT` which is needed so that we can map
/// device memory later in the example.
...
fn create_buffer(
    ...
    // Since we allocated HOST_VISIBLE | HOST_COHERENT memory, we can map it directly.
    memory
        .map(vulkano::memory::MemoryMapInfo {
            offset: 0,
            size,
            ..Default::default()
        })
        .expect("Memory should be mappable since we allocated HOST_VISIBLE memory");

    // Write test input to the mapped memory.
    unsafe {
        let mapping_state = memory
            .mapping_state()
            .expect("Memory should be mapped after calling map()");
        let ptr = mapping_state
            .slice(0..size)
            .expect("Slice range should be within bounds")
            .as_ptr() as *mut u8;
        let content = std::slice::from_raw_parts_mut(ptr, size as usize);
        generate_test_input(content, width, height, i, i_max);
    }

    // Unmap the memory before exporting.
    memory
