...
/// NVDEC/CUVID decoder library
#[cfg(unix)]
const NVCUVID_LIB: (&str, &str) = ("nvcuvid", "libnvcuvid.so");
#[cfg(windows)]
const NVCUVID_LIB: (&str, &str) = ("nvcuvid", "nvcuvid.lib");

/// CUDA runtime library (needed for decoder)
#[cfg(unix)]
const CUDA_LIB: (&str, &str) = ("cuda", "libcuda.so");
#[cfg(windows)]
const CUDA_LIB: (&str, &str) = ("cuda", "cuda.lib");
...
fn main() {
    ...
    // Link to NVENC library.
    println!("cargo:rustc-link-lib={}", NVENC_LIB.0);

    // Link to NVDEC/CUVID library.
    println!("cargo:rustc-link-lib={}", NVCUVID_LIB.0);

    // Link to CUDA runtime library.
    println!("cargo:rustc-link-lib={}", CUDA_LIB.0);

    // Add the first found candidate location to link search.
    // Try to find NVENC library path
    match library_candidates_for(NVENC_LIB.1).next() {
        ...
        None => panic!(
            "Could not find NVIDIA Video Codec SDK libraries.\nPlace the libraries where you have \
             ...
             your installation so that `$NVIDIA_VIDEO_CODEC_SDK_PATH/{}` is a valid path to the library file.",
            NVENC_LIB.1
        ),
    }

    // Also add CUVID library path if different
    if let Some(path) = library_candidates_for(NVCUVID_LIB.1).next() {
        println!("cargo:rustc-link-search={}", path.display());
    }

    // Also add CUDA library path if different
    if let Some(path) = library_candidates_for(CUDA_LIB.1).next() {
        println!("cargo:rustc-link-search={}", path.display());
    }
}
...
/// Look for directories which contain the specified library file.
fn library_candidates_for(lib_name: &'static str) -> impl Iterator<Item = PathBuf> {
    ENVIRONMENT_VARIABLES
        ...
        .flat_map(move |root| {
            ...
            LIBRARY_CANDIDATES
                ...
                .filter(move |path| path.join(lib_name).is_file())
        })
}
