...
fn main() -> Result<()> {
    ...
    // Create controllers
    let mut leader = Xl320Controller::new().with_serial_port(leader_port);

    let mut follower = Xl320Controller::new().with_serial_port(Box::new(follower_port));
    ...
    for id in SERVO_IDS {
        if let Err(e) = leader.write_torque_enable(id, 0) {
            println!(
    ...
    for id in SERVO_IDS {
        if let Err(e) = follower.write_torque_enable(id, 0) {
            println!(
    ...
    for id in SERVO_IDS {
        let _ = follower.write_torque_enable(id, 0);
    }
